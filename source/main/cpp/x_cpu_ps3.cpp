...
namespace xcore
{
	namespace cpu
	{
		...
		static void initialize()
		{
			...
			_scale.tsc2nsec = (1000ULL << TSC2NSEC_SCALE_SHIFT);
			_scale.tsc2usec = (1ULL    << TSC2USEC_SCALE_SHIFT);
			_scale.nsec2tsc = (1 << NSEC2TSC_SCALE_SHIFT) / 1000ULL;
			_scale.usec2tsc = (1 << USEC2TSC_SCALE_SHIFT);
		}
		...
		namespace cpu_info_sys
		{
			...
			enum Manufacturer 
			{
				AMD, Intel, NSC, UMC, Cyrix, NexGen, IDT, Rise, Transmeta, Sony, UnknownManufacturer
			};
			...
			static void sInitialize()
			{
				...
				x_strcpy(ChipID.ProcessorName, sizeof(ChipID.ProcessorName), "Cell Broadband Engine, altivec supported");
				x_strcpy(ChipID.Vendor, sizeof(ChipID.Vendor), "Sony");
		...
		static u64 sCPUSpeedInkHz = 3.2 * 1000 * 1000; 
