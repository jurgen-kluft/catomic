/**
 * @file xatomic\private\x_atomic_arm_3ds.h
 * 3DS ARM 32 bit specific implementation of the atomic operations.
...
namespace xcore
{
	namespace atomic
	{
		// 32 and 64 bit interlocked compare and exchange functions for the 3DS ARM11 SoC
		namespace cpu_arm_3ds
		{
			inline static u32 sInterlockedCompareExchange(volatile u32 *dest, u32 exchange, u32 comperand)
			{
				u32 old = (u32)nn::fnd::ARMv6::Interlocked::CompareAndSwap((s32*)dest, comperand, exchange);
				return old;
			}

			inline static bool sInterlockedSetIfEqual(volatile u32 *dest, u32 exchange, u32 comperand)
			{
				u32 old = nn::fnd::ARMv6::Interlocked::CompareAndSwap((s32*)dest, comperand, exchange);
			...
			inline static void sWrite(volatile u32 *dest, u32 v)
			{
				for (;;)
				{
			...
			{
				u64 old = nn::fnd::ARMv6::Interlocked::CompareAndSwap((u64*)dest, comperand, exchange);
				return old;
			}
		...
		namespace cpu_interlocked = cpu_arm_3ds;
